...
union Logger_Class {
  ...
  struct {
    struct Class Class;
    void (*_private log)(union Logger *const logger, enum LogLevel const level,
...
extern void Logger_populate(union Logger *const logger,
                            union Formatter *const formatter,
                            union LoggerHandler *const handler);

extern void Logger_log(union Logger *const logger, enum LogLevel const level,
                       uint32_t const line_num, char const *const file_name,
