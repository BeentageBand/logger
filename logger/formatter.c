...
static char *formatter_format(union Formatter *const this,
                              enum LogLevel const level,
                              uint32_t const line_num,
                              char const *const file_name,
                              char const *const msg, va_list va_args);
...
char *formatter_format(union Formatter *const this, enum LogLevel const level,
  ...
  int pos = sprintf(formatted_msg, "%s:%d [%s] %s:", file_name, line_num,
                    level_str, this->facility_name);
  if (pos < 0)
    return NULL;
...
void Formatter_populate(union Formatter *const this,
                        char const *const facility_name,
                        enum LogLevel const log_level,
