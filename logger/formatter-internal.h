...
char const *Formatter_format(union Formatter *const formatter,
                             enum LogLevel const level, uint32_t const line_num,
                             char const *const file_name, char const *const msg,
                             va_list const va_args) {
  return formatter->vtbl->format(formatter, level, line_num, file_name, msg,
                                 va_args);
}
